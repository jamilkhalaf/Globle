...
const ShapleOnline = ({ matchData, onAnswerSubmit, gameState, gameTimer }) => {
  const [shapeGuess, setShapeGuess] = useState('');
  ...
  const handleShapeSubmit = () => {
    if (!shapeGuess.trim()) return;
    
    onAnswerSubmit({
      answer: shapeGuess.trim(),
      timeTaken: 60 - gameTimer
    });
    
    setAttempts(prev => [...prev, shapeGuess.trim()]);
    setShapeGuess('');
  };
    ...
    <Box sx={{ textAlign: 'center', mb: 4 }}>
      ...
      <Box sx={{ mb: 3 }}>
        <TextField
          ...
          value={shapeGuess}
          onChange={(e) => setShapeGuess(e.target.value)}
      ...
      <Button
        ...
        disabled={!shapeGuess.trim() || gameEnded}
        sx={{ 
          bgcolor: '#43cea2',
