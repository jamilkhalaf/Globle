...
const USOnline = ({ matchData, onAnswerSubmit, gameState, gameTimer }) => {
  ...
  const handleStateSubmit = () => {
    if (!stateGuess.trim()) return;
    
    onAnswerSubmit({
      answer: stateGuess.trim(),
      timeTaken: 60 - gameTimer
    });
    
    setAttempts(prev => [...prev, stateGuess.trim()]);
    setStateGuess('');
  };
    ...
    <Box sx={{ textAlign: 'center', mb: 4 }}>
      ...
      <Box sx={{ mb: 3 }}>
        <TextField
          ...
          value={stateGuess}
          onChange={(e) => setStateGuess(e.target.value)}
      ...
      <Button
        ...
        disabled={!stateGuess.trim() || gameEnded}
        sx={{ 
          bgcolor: '#43cea2',
