...
const CapitalsOnline = ({ matchData, onAnswerSubmit, gameState, gameTimer }) => {
  ...
  const handleCapitalSubmit = () => {
    if (!capitalGuess.trim()) return;
    
    onAnswerSubmit({
      answer: capitalGuess.trim(),
      timeTaken: 60 - gameTimer
    });
    
    setAttempts(prev => [...prev, capitalGuess.trim()]);
    setCapitalGuess('');
  };
  ...
  const renderGameInterface = () => (
    <Box sx={{ textAlign: 'center', mb: 4 }}>
      ...
      <Button
        ...
        disabled={!capitalGuess.trim() || gameEnded}
        sx={{ 
          bgcolor: '#43cea2',
